...
- **Exploring or operating a worked example** → work inside its folder under `examples/`.
  ...
  its board back to BotBeam), `claude-manager` (a global Claude Code manager — a live
  **bound** lens over `~/.claude/` with modules for sessions, usage, and more; contained
  state for summaries/archives; the one workspace that also grows write **actions** and
  a multi-view nav — port 8769).
