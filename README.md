...
| **[`examples/`](examples/)** | Complete worked examples, fully runnable: **[`lit-review`](examples/lit-review/)** (a client-commissioned literature review, from empty folders to the delivered report), **[`tablethat`](examples/tablethat/)** (an AI table builder — Build → Populate → Enrich — replicated as a workspace), **[`newsletter`](examples/newsletter/)** (a weekly literature-monitoring newsletter: query → filter → summarize → assemble), **[`life-tracker`](examples/life-tracker/)** (a personal life-infrastructure tracker that beams its board back to BotBeam), and **[`claude-manager`](examples/claude-manager/)** (a global Claude Code manager — a *bound* lens over `~/.claude/` with modules for sessions, usage, and more; the example that exercises contained state, write actions, and a multi-view nav). | [examples/lit-review/JOURNEY.md](examples/lit-review/JOURNEY.md) |
