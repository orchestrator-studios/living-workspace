...
| **[`examples/`](examples/)** | Complete worked examples, fully runnable: **[`lit-review`](examples/lit-review/)** (a client-commissioned literature review, from empty folders to the delivered report), **[`tablethat`](examples/tablethat/)** (an AI table builder — Build → Populate → Enrich — replicated as a workspace), **[`newsletter`](examples/newsletter/)** (a weekly literature-monitoring newsletter: query → filter → summarize → assemble), **[`life-tracker`](examples/life-tracker/)** (a personal life-infrastructure tracker that beams its board back to BotBeam), and **[`session-tracker`](examples/session-tracker/)** (a live dashboard over every Claude Code session on the machine — a *bound, read-only* lens that owns no data). | [examples/lit-review/JOURNEY.md](examples/lit-review/JOURNEY.md) |
